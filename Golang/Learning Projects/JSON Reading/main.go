package main

import (
	"encoding/json"
	"fmt"
	"io/ioutil"
	"os"
	"strconv"
)

func main() {
	//Open our jsonFile
	jsonFile, err := os.Open("users.json")

	//if our os.Open returns an error, handle it
	if err != nil {
		fmt.Println(err)
		return
	}

	//read our opened jsonFile as a byte array
	byteValue, _ := ioutil.ReadAll(jsonFile)

	// we initialize our Users array
	var users Users

	// we unmarshal our byteArray which contains our jsonFile's content into 'users' which we defined above
	json.Unmarshal(byteValue, &users)

	// we iterate through ever user within our users array and print out the information
	for i := 0; i < len(users.Users); i++ {
		fmt.Println("User Type: " + users.Users[i].Type)
		fmt.Println("User Age: " + strconv.Itoa(users.Users[i].Age))
		fmt.Println("User Name: " + users.Users[i].Name)
		fmt.Println("Facebook URL: " + users.Users[i].Social.Facebook)
	}

	//defer the closing our jsonFile so that we can parse it later on
	defer jsonFile.Close()
}

// Users struct which contains an array of users
type Users struct {
	Users []User `json:"users"`
}

// User struct which contains a name, a type and a list of social links
type User struct {
	Name   string `json:"name"`
	Type   string `json:"type"`
	Age    int    `json:"Age"`
	Social Social `json:"social"`
}

// Social struct which contains a list of links
type Social struct {
	Facebook string `json:"facebook"`
	Twitter  string `json:"twitter"`
}
